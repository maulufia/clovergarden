...
class agspay40
{
	...
	*/
	...
	{
		$result = isset($this->RESULT[$name]) ? $this->RESULT[$name] : null;
		if( strlen($result) == 0 || $result == "") $result = isset($this->REQUEST[$name]) ? $this->REQUEST[$name] : null; // 정말 코드 엉망이다.
		return $result;
	}
...
class PayLog 
{
	...
	function CloseLog($msg)
	{
		...
		$this->WriteLog( INFO, "END " . TYPE . " ".$msg." Transaction time:[".round($Transaction_time,3)."sec]" ); // $this->REQUEST["Type"] 이런게 어딨나요?
		$this->WriteLog( INFO, "===============================================================" );
		fclose( $this->log_fd );
	}
}
...
function GetTime()
{
    list($sec1, $sec2) = explode(" ", microtime(false)); // true여서 에러가 났었음
