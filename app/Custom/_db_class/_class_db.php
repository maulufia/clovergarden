<?php
    /*------------------------------------------------------------------------------------------------------*
     * @copyright   : clovergarden
     * @description : database process 클래스
     * @author      : 장성훈 to_shin@nate.com
     * @created     : 2015.03
     *------------------------------------------------------------------------------------------------------*/
    ...
    class DBClass
    {
        ...
        */
        ...
        function PageListCount($pTableName, $pWhere, $pSearchKey, $pSearchVal, $pConnect=null)
        {
			...
			if($pTableName == "new_tb_email_send"){
	            $sql = 'select count(send_date) as counts from '.$pTableName.' '.$pWhere;
			} else {
        ...
        function PageList2($pTableName, $pClass, $pWhere, $pSearchKey, $pSearchVal, $pOrderBy, $pSubSql, $pPageNo, $pPageView, $pJoin=null, $pConnect=null)
        {
            if($pSearchKey != '' && $pSearchVal != ''){
                if($pWhere) $pWhere = $pWhere.' and ';
                else $pWhere = ' where ';
            ...
            $sql = 'select * '.$pSubSql.' from '.$pTableName.' '.$pWhere.' '.$pOrderBy.' limit 0 , '.$pPageView;
        ...
        function InsertMultiDB2($pTableName, $pField, $pArr, $pConnect=null)
        {
            for($i=0, $cnt=count($pArr); $i < $cnt; $i++) {
                $multiValue = 'insert into '.$pTableName.' ('.join(',',$pField).") values"."('".join("','",$pArr[$i])."');";

				//echo $multiValue."<BR>";
				//@mysql_query($sql, $pConnect);
				
				@mysql_query($multiValue, $this->obj_db_conn);
            }
            $sql = $sql.substr($multiValue,1);
