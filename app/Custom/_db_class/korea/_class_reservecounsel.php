<?php
    settype($nReservecounsel, 'object');

    class ReservecounselClass
    {
        var $seq;               //고유번호
        ...
        function ReservecounselClass()
        {
            $this->table_name = 'new_tb_kor_reservecounsel';
            $this->sub_sql    = '';
        ...
        */
		...
		function ArrList($pResult, $pCnt, $pJoin=null)
        {
			...
			$pResultValue[$pCnt]->date       = stripcslashes($pResult['date']);
			$pResultValue[$pCnt]->time       = stripcslashes($pResult['time']);
            ...
            $pResultValue[$pCnt]->part    = stripcslashes($pResult['part']);
        ...
        function ArrReservecounsel($pVal, $pName, $pOption, $pKind, $pWrite='')
        {
            switch($pKind)
            {
                case 'search' :
				...
				case 'gender' :
                    ...
                    break;
				case 'part' :
                    $SelectField = array("수술 전 상담","수술 후 외래");
                    $SelectValue = array('1','2');
                    break;
            }
            if($pWrite == ''){
