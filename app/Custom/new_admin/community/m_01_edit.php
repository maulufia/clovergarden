...
$Conn = new DBClass();
    ...
    $nTimeline->where = "where seq ='".$seq."'";
    $nTimeline->read_result = $Conn->AllList($nTimeline->table_name, $nTimeline, "*", $nTimeline->where, null, null);

    if(count($nTimeline->read_result) != 0){
        $nTimeline->VarList($nTimeline->read_result, 0, null);
    }else{
        $Conn->DisConnect();
        ...
        <div id="right_area">
            ...
            <table class="bbs-write">
                ...
                <tr>
                    <th>제목</th>
                    <td colspan="3">
                        <input type="text" name="subject" style="width:300px;" value="<?=$nTimeline->subject?>"/>
                    </td>
                </tr>
				<tr>
                    <th>작성자</th>
                    <td colspan="3">
                        <input type="text" name="writer_name" style="width:100px;" value="<?=$nTimeline->writer_name?>"/>
                    </td>
                </tr>
                </tbody>
            </table>
