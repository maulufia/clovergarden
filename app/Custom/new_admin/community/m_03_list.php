        ...
        <div id="right_area">
            ...
            <div class="bbs-search" style="margin-bottom:0px;">
                <form name="frm" method="post" action="<?=$list_link?>" style="display:inline">
                    <?$nBanner->ArrBanner(null, "name='search_key'", null, 'search')?>
                ...
                <tr>
					...
					<?php
                        if(FileExists('../../up_file/banner/'.$nBanner->file_edit[1])){
                            echo "<img src='../../up_file/banner/".$nBanner->file_edit[1]."' border='0'>";                  
                        }
                    ?>
					</td>
                    <td class="subject">
                        <a href="javascript:pageLink('<?=$nBanner->seq?>','<?=$row_no?>','view','<?=$view_link?>');"><?=$nBanner->subject?></a>
                    </td>
					<td><?=$nBanner->hit?></td>
