...
$Conn = new DBClass();
    ...
    $nSCompany->where = "where seq ='".$seq."'";
    $nSCompany->read_result = $Conn->AllList($nSCompany->table_name, $nSCompany, "*", $nSCompany->where, null, null);

    if(count($nSCompany->read_result) != 0){
        $nSCompany->VarList($nSCompany->read_result, 0, null);
    }else{
        $Conn->DisConnect();
        ...
        <div id="right_area">
            ...
            <table class="bbs-write">
				...
				<tr>
                    <th>업체명</th>
                    <td colspan="2">
                        <input type="text" name="subject" style="width:100px;" value="<?=$nSCompany->subject?>"/>
                    </td>
                </tr>        
				<tr>
                    <th style="vertical-align:middle;">내용</th>
                    <td colspan="2">
					<textarea name="content"><?=$nSCompany->content?></textarea>
                    </td>
                </tr>
                <tr>
                    <th>썸네일</th>
                    <td colspan="3">
                        ...
                        ?>
                        <span class="lmits">(<?=$nSCompany->file_mime_type[1]?> : <?=$nSCompany->file_volume[1]?><?=LOW_FILESIZE?>)</span>
                    </td>
                </tr>
            ...
            <?=SubmitHidden()?>
            <input type="hidden" name="file_real1" value="<?=$nSCompany->file_real[1]?>"/>
            <input type="hidden" name="file_edit1" value="<?=$nSCompany->file_edit[1]?>"/>
            <input type="hidden" name="file_byte1" value="<?=$nSCompany->file_byte[1]?>"/>
            </form>
            <div class="btn-area">
                <div class="fleft">
