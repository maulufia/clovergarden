...
$Conn = new DBClass();
	...
	if($date_search == "1"){
	    $nKakaocounsel->where = " where (substring(reg_date,1,10) >= cast('".$start_date."' AS DATE) and substring(reg_date,1,10) <= cast('".$end_date."' AS DATE))";
	}

    $nKakaocounsel->total_record = $Conn->PageListCount
    (
        $nKakaocounsel->table_name, $nKakaocounsel->where, $search_key, $search_val
    );
    $nKakaocounsel->page_result = $Conn->PageList
    (
        $nKakaocounsel->table_name, $nKakaocounsel, $nKakaocounsel->where, $search_key, $search_val, 'order by seq desc', $nKakaocounsel->sub_sql, $page_no, $nKakaocounsel->page_view
    );
    ...
    <div id="container">
        ...
        <div id="right_area">
            ...
            <div class="bbs-search" style="margin-bottom:0px;">
				<?$nKakaocounsel->ArrKakaocounsel(null, "name='search_key'", null, 'search')?>
...
<?php
    if(count($nKakaocounsel->page_result) > 0){
        $row_no = $nKakaocounsel->total_record - ($nKakaocounsel->page_view * ($page_no - 1));
        for($i=0, $cnt_list=count($nKakaocounsel->page_result); $i < $cnt_list; $i++) {
            $nKakaocounsel->VarList($nKakaocounsel->page_result, $i, null);
?>
                <tr>
                    <td><input type="checkbox" name="delete_seq[]" value="<?=$nKakaocounsel->seq?>" /></td>
                    <td><?=$row_no?></td>
					<td><?$nKakaocounsel->ArrKakaocounsel($nKakaocounsel->category, null, null, 'category',1)?></td>
                    <td><?=$nKakaocounsel->name?></td>
                    <td><?=$nKakaocounsel->cell?></td>
                    <td><?=$nKakaocounsel->reg_date?></td>
                    <td>
						<?$nKakaocounsel->ArrKakaocounsel($nKakaocounsel->counsel_step, "name='edit_counsel_step[]'", null, 'counsel_step')?>
                        <?$nKakaocounsel->ArrKakaocounsel($nKakaocounsel->counsel_state, "name='edit_counsel_state[]'", null, 'counsel_state')?>
                    </td>
                </tr>
				<tr>
					 <td colspan="2" style="text-align:center">비고</td>
				     <td colspan="6" class="subject">
						<span>
						 <textarea type="text" name="info" id="info<?=$nKakaocounsel->seq?>" style="width:750px;" maxlength="200"><?=$nKakaocounsel->info?></textarea><a href="javascript:infoEdit(<?=$nKakaocounsel->seq?>);"><img src="/new_admin/images/btn_small_modify.gif" alt="수정" align="center"/></a>
						<input type="hidden" name="edit_seq[]" value="<?=$nKakaocounsel->seq?>" />
						</span>
					 </td>
				</tr>
                <tr>
                    ...
                    <td class="subject" colspan="8" style="background-color:#C9D6F0;">
                        <div style="color:#000"><?=strip_tags(stripcslashes($nKakaocounsel->content))?></div>
                        <div id="content_view<?=$i?>" style="display:none;padding:5px;">
                            <textarea style="width:100%;height:100%" disabled><?=$nKakaocounsel->content?></textarea>
                        </div>
                    </td>
                </tr>
				<?
            ...
            <?php
                if($nKakaocounsel->total_record != 0){
                    $nPage = new PageOut();
                    $nPage->AdminPageList($nKakaocounsel->total_record, $page_no, $nKakaocounsel->page_view, $nKakaocounsel->page_set, $nKakaocounsel->page_where, 'pageNumber');
                }
            ?>
            </div>
