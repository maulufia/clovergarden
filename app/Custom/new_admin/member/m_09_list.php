...
$Conn = new DBClass();
	...
	if($_GET[mod_mode] == "modify_seq"){
		...
		mysql_query($sql);
			...
			<script>
			alert('정보가 수정되었습니다.');
			window.location='./m_09_list.php';
			</script>
			";

	}
	if($_GET[mclover_update_type] == "delete"){

		$sql = "update ".$nMember->table_name." set clover_seq='', clover_seq_adm='', clover_seq_adm_type='' where seq='".$_GET[seq]."'";
		mysql_query($sql);
			echo "
			<script>
			alert('정보가 삭제되었습니다.');
...
?>
                <tr>
					...
					if($ex_date_or_type[1] == "ok"){
						echo "<a href='./m_09_list.php?mod_mode=modify_seq&modify_v=".$ex_date_or_type[0]."&mod_seq=".$nMember->seq."'><font color='blue'><B>처리</B><BR>(클릭시 미처리로 변경됨)</font></a>";
					} else {
						echo "<a href='./m_09_list.php?mod_mode=modify_seq&modify_v=".$ex_date_or_type[0]."[@@]ok&mod_seq=".$nMember->seq."'><font color='red'>미처리<BR>(클릭시 처리로 변경됨)</font></a>";
					}
					?>
					</td>
					<td><input type="button" value="삭제" style="padding:5px;" onclick="if(confirm('삭제하시겠습니까?')){window.location = './m_09_list.php?seq=<?=$nMember->seq?>&mclover_update_type=delete'; }"></td>
                </tr>
<?
            $row_no = $row_no - 1;
        }
