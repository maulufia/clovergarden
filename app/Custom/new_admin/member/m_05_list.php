...
<?
	...
	$nEmail->where  = "where send_ck = 'nomember'";
...
$Conn = new DBClass();
    ...
    (
        $nEmail->table_name, "where send_ck = 'nomember' group by send_date", $search_key, $search_val
    );
    ...
    if(count($nEmail->page_result) > 0){
        ...
        for($i=0, $cnt_list=count($nEmail->page_result); $i < $cnt_list; $i++) {
			...
			$Conn = new DBClass();
				...
				(	
					$nEmail_2->table_name, $nEmail_2, "*", "where send_ck = 'nomember' and send_date='".$nEmail->send_date."'", null, null
				);
