...
<?php
	...
	$excel_link = 'm_02_excel.php';
	$excel_link2 = 'm_02_excel2.php';
?>
...
$Conn = new DBClass();
    ...
    $nMember->where = " where (user_state = '2' or user_state='-1' or user_state='5') and clover_win='S' $search_sql";
    ...
    $nMember->where = " where (user_state = '2' or user_state='-1' or user_state='5') $search_sql";
...
if($_GET[mmode] == "cwmode"){
	$sql = "update new_tb_member set clover_win='S' where seq = '".$_GET[mseq]."'";
	mysql_query($sql);
	echo "
	<script>
	alert('기부왕이 선택되었습니다.');
	window.location = './m_02_list.php';
	</script>

	";
}

if($_GET[mmode] == "cdmode"){
	$sql = "update new_tb_member set clover_win='' where seq = '".$_GET[mseq]."'";
	mysql_query($sql);
	echo "
	<script>
	alert('기부왕이 삭제되었습니다.');
...
?>
				<td>
					...
					<tr height=30>
						<td align=center><input type="button" value="삭제" onclick="window.location='./m_02_list.php?mmode=cdmode&mseq=<?=$nMember_win->seq?>';" style="border:1px solid #e8e8e8; padding:3px;background:#3952a8; font-weight:bold; color:#fff;"></td>
					</tr>
					</table>
				</td>
<?
...
?>
                <tr <?if($nMember->update_ck == 'Y'){?>bgcolor="eeeeee"<?}?>>
                    <td>
                    ...
                    <td><?=$row_no?></td>
                    <td><?if($nMember->user_state==-1) echo "<font style='color:red'>[탈퇴회원]</font>";?><a href="javascript:pageLink('<?=$nMember->seq?>','<?=$row_no?>','','<?=$edit_link?>');"><?=$nMember->user_id?></a></td>					
					...
					<td><?=$nMember->user_email?><?=$use_point?></td>
                    <td><?=str_replace('-','.',substr($nMember->reg_date,0,10))?></td>
                    <td>
					<?if($ck_clover_win == "S"){?>
						<?if($nMember->clover_win == "S"){?>
						선택된회원
						<?}else{?>
					...
					<?}else{?>
						...
						선택된회원
						<?}else{?>
						<input type="button" value="선택" onclick="window.location='./m_02_list.php?mmode=cwmode&mseq=<?=$nMember->seq?>';" style="border:1px solid #e8e8e8; padding:3px;background:#3952a8; font-weight:bold; color:#fff;">
						<?}?>
					<?}?>
					</td>
                </tr>
...
?>
			...
			<table cellpadding=0 cellspacing=0 border=0 width=100% align=center>
			<tr>
				<td align=center width=10%><input type="button" value="엑셀다운로드" onclick="window.location='./m_02_exec_down.php';" style="border:1px solid #e8e8e8; padding:5px;background:#3952a8; font-weight:bold; color:#fff;"></td>
				<td align=center>
...
//-->
		...
		<form name="form_submit_point_ck" method="get" action="<?=$PHP_SELF?>" style="display:inline">
        ...
        <form name="form_submit_ckdel" method="post" action="./m_02_del.php?cktype=member" style="display:inline">
            <input type="hidden" name="s_mode" value="ch_send">
			<input type="hidden" name="s_email" value="">
			<input type="hidden" name="s_name" value="">
        </form>
