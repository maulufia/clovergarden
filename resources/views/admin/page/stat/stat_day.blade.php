...
?>
    ...
    <script type="text/javascript" src="/js/new_js/highcharts_plugin/jquery.highcharts.js"></script>
    <script type="text/javascript" src="/js/new_js/highcharts_plugin/jquery.exporting.js"></script>
    <script language="javascript">
        ...
        $(document).ready(function()
        {
            ...
            $('#end_date').datepicker({showOn: "both", buttonImageOnly: true, buttonImage: "/imgs/new_images/calendar.png", numberOfMonths: 1, showButtonPanel: true, showOtherMonths: true, selectOtherMonths: true, changeMonth: true, changeYear: true});
            ...
            if((date2 - date1) <= 518400000 ){
                ...
                chart = new Highcharts.Chart({
                    ...
                    xAxis: {
                        ...
                        <?php
                        $comma = null;
                        $chart_start_cnt = null;
                        $chart_pre_start_cnt = null;
                       ...
                       for($i=0, $cnt_list=dateDiff($end_date, $start_date); $i <= $cnt_list; $i++) {
                            ...
                            ${$cnt_view_day.'_cnt'} = isset(${$cnt_view_day.'_cnt'}) ? ${$cnt_view_day.'_cnt'} : null;
                            $chart_start_cnt = $chart_start_cnt.$comma.${$cnt_view_day.'_cnt'};
                            
                            ${'pre_'.$cnt_pre_view_day.'_cnt'} = isset(${'pre_'.$cnt_pre_view_day.'_cnt'}) ? ${'pre_'.$cnt_pre_view_day.'_cnt'} : null;
        ...
        <div id="right_area">
            ...
            <div class="bbs-search" style="margin-bottom:0px;">
                <form name="frm" method="post" action="{{ $list_link }}" style="display:inline">
                    ...
                    <a href="javascript:sendSubmit()"><img src="/imgs/admin/images/btn_search.gif" alt="search"/></a>
                </form>
            </div>
            <table id="stats" class="bbs-stats">
                ...
                <tr>
                    <td>{{ $view_day }}</td>
                    <td><?php if(${$view_day.'_cnt'}){ echo number_format(${$view_day.'_cnt'}); } else { echo 'ㆍ';} ?></td>
                    <td>{{ $pre_view_day }}</td>
                    <td><?php if(${'pre_'.$pre_view_day.'_cnt'}){ echo number_format(${'pre_'.$pre_view_day.'_cnt'}); } else { echo 'ㆍ';} ?></td>
                </tr>
                <?php
                }
                ?>
