       ...
       <tbody>
        ...
        if(count($nClover->page_result) > 0){
          ...
          for($i=0, $cnt_list=count($nClover->page_result); $i < $cnt_list; $i++) {
            ...
            <td class="subject">
              <a href="{{ route('admin/clover', array('item' => 'banner', 'seq' => $nClover->seq, 'row_no' => $row_no, 'type' => 'view')) }}">{{ $nClover->subject }}</a>
            </td>
          </tr>
          <?php
          $row_no = $row_no - 1;
...
<div class="btn-area tmargin">
  ...
  <div class="fright">
    <a href="{{ route('admin/clover', array('item' => 'banner', 'type' => 'write')) }}"><img src="/imgs/admin/images/btn_write.gif" alt="writing" /></a>
  </div>
</div>
</div>
<!-- //right_area -->
