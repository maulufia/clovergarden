        ...
        <div id="right_area">
            ...
            <div class="bbs-search" style="margin-bottom:0px;">
                <form name="frm" method="post" action="{{ $list_link }}" style="display:inline">
                    <?php $nSponsor->ArrSponsor(null, "name='search_key'", null, 'search') ?>
...
?>
          ...
          <td>{{ $row_no }}</td>
					<td>
            <img src='/imgs/up_file/sponsor/{{ $nSponsor->file_edit[1] }}' border='0'>
					</td>
          <td class="subject">
              <a href="{{ route('admin/sponsor', array('item' => 'companion', 'seq' => $nSponsor->seq, 'row_no' => $row_no, 'type' => 'view')) }}">{{ $nSponsor->subject }}</a>
          </td>
					<td>{{ isset($nSponsor->hit) ? $nSponsor->hit : null }}</td>
