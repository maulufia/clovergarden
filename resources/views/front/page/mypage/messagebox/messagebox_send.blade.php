	...
	<article class="brd_list">
		...
		<table>
			...
			<tr class="title">
				...
				<th scope="col"><span>받는사람</span></th>
			...
			<?php
				if(count($nMessage->page_result) > 0){
					...
					for($i=0, $cnt_list=count($nMessage->page_result); $i < $cnt_list; $i++) {
						...
						$receive = explode(',',$nMessage->receive_id);

			?>
			<tr>
				<td class="no">{{ $row_no }}</td>
				<td class="normal">{{ isset($receive[1]) ? $receive[1] : null }}</td>
				<td class="subject"><?php echo $nMessage->content; ?> <font color="#fd4f00"><?php if($nMessage->hit>0) echo "[읽음]"; else echo "[읽지않음]"; ?></font><!-- </a> --></td>
