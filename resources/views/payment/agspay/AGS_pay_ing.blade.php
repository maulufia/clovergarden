...
********************************************************************************/
	...
	****************************************************************************/
	...
	$agspay->SetValue("UserEmail",trim($_POST["UserEmail"])."@".trim(isset($_POST["UserEmail2"]) ? $_POST["UserEmail2"] : null));	//주문자이메일
	...
	$agspay->SetValue("OrdPhone",trim($_POST["OrdPhone"])."-".trim($_POST["OrdPhone2"])."-".trim($_POST["OrdPhone3"]));		//주문자연락처
	...
	$agspay->SetValue("VIRTUAL_DEPODT",trim(isset($_POST["VIRTUAL_DEPODT"]) ? $_POST["VIRTUAL_DEPODT"] : null));		//입금예정일(가상계좌)
...
<form name=frmAGS_pay_ing method="POST" accept-charset="EUC-KR" action="{{ route('clovergarden', array('cate' => 1, 'dep01' => 0, 'dep02' => 0, 'type' => 'write_result', 'seq' => 2)) }}"> 
