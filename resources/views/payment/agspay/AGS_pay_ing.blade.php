...
********************************************************************************/
	...
	****************************************************************************/
	...
	$agspay->SetValue("AgsPayHome", app_path() . "/Libraries/agspay");			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	$agspay->SetValue("UseNetCancel","true");					//true : 망취소 사용. false: 망취소 미사용 // !! 순서가 이상함. AuthTy 값 설정후 이 라인 실행해줘야 함
	...
	//$agspay->SetValue("UserEmail",trim($_POST["UserEmail"]));	//주문자이메일 (수정)
	...
	$agspay->SetValue("OrdAddr", trim($_POST["OrdAddr"] . "-" . $_POST["OrdAddr2"] . " " . $_POST["addr1"] . " " . $_POST["addr2"]));		//주문자주소 가상계좌추가
	// $agspay->SetValue("RcpNm",trim($_POST["RcpNm"]));			//수신자명
	$agspay->SetValue("RcpNm", trim($_POST["OrdNm"]));			//수신자명
	// $agspay->SetValue("RcpPhone",trim($_POST["RcpPhone"]));		//수신자연락처
	$agspay->SetValue("RcpPhone", trim($_POST["OrdPhone"]));		//수신자연락처
	// $agspay->SetValue("DlvAddr", trim($_POST["DlvAddr"]));		//배송지주소
	$agspay->SetValue("DlvAddr", trim($_POST["OrdAddr"] . "-" . $_POST["OrdAddr2"] . " " . $_POST["addr1"] . " " . $_POST["addr2"]));		//배송지주소
	...
	$agspay->SetValue("HP_ID",trim(isset($_POST["HP_ID"]) ? $_POST["HP_ID"] : null));					//HP_ID(핸드폰결제)
	$agspay->SetValue("HP_SUBID",trim(isset($_POST["HP_SUBID"]) ? $_POST["HP_SUBID"] : null));				//HP_SUBID(핸드폰결제)
	$agspay->SetValue("HP_UNITType",trim(isset($_POST["HP_UNITType"]) ? $_POST["HP_UNITType"] : null));		//HP_UNITType(핸드폰결제)
	...
	$agspay->SetValue("VIRTUAL_DEPODT",trim(!empty($_POST["VIRTUAL_DEPODT"]) ? $_POST["VIRTUAL_DEPODT"] : date("Ymd") + 5));		//입금예정일(가상계좌)
	$agspay->SetValue("ZuminCode",trim($_POST["ZuminCode"]));				//주민번호(가상계좌)
	$agspay->SetValue("MallPage", trim(isset($_POST["MallPage"]) ? $_POST["MallPage"] : null));					//상점 입/출금 통보 페이지(가상계좌)
...
<!-- 각 결제 공통 사용 변수 -->
<input type=hidden name=AuthTy value="{{ iconv('EUC-KR', 'UTF-8', $AuthTy) }}">		<!-- 결제형태 -->
<input type=hidden name=SubTy value="{{ iconv('EUC-KR', 'UTF-8', $SubTy) }}">			<!-- 서브결제형태 -->
<input type=hidden name=rStoreId value="{{ iconv('EUC-KR', 'UTF-8', $rStoreId) }}">	<!-- 상점아이디 -->
<input type=hidden name=rOrdNo value="{{ iconv('EUC-KR', 'UTF-8', $rOrdNo) }}">		<!-- 주문번호 -->
<input type=hidden name=rProdNm value="{{ iconv('EUC-KR', 'UTF-8', $rProdNm) }}">		<!-- 상품명 -->
<input type=hidden name=rAmt value="{{ iconv('EUC-KR', 'UTF-8', $rAmt) }}">			<!-- 결제금액 -->
<input type=hidden name=rOrdNm value="{{ iconv('EUC-KR', 'UTF-8', $rOrdNm) }}">		<!-- 주문자명 -->
<input type=hidden name=AGS_HASHDATA value="{{ $AGS_HASHDATA }}">				<!-- 암호화 HASHDATA -->

<input type=hidden name=rSuccYn value="{{ iconv('EUC-KR', 'UTF-8', $rSuccYn) }}">	<!-- 성공여부 -->
<input type=hidden name=rResMsg value="{{ iconv('EUC-KR', 'UTF-8', $rResMsg) }}">	<!-- 결과메시지 -->
<input type=hidden name=rApprTm value="{{ iconv('EUC-KR', 'UTF-8', $rApprTm) }}">	<!-- 결제시간 -->

<!-- 신용카드 결제 사용 변수 -->
<input type=hidden name=rBusiCd value="{{ iconv('EUC-KR', 'UTF-8', $rBusiCd) }}">		<!-- (신용카드공통)전문코드 -->
<input type=hidden name=rApprNo value="{{ iconv('EUC-KR', 'UTF-8', $rApprNo) }}>">		<!-- (신용카드공통)승인번호 -->
<input type=hidden name=rCardCd value="{{ iconv('EUC-KR', 'UTF-8', $rCardCd) }}">	<!-- (신용카드공통)카드사코드 -->
<input type=hidden name=rDealNo value="{{ iconv('EUC-KR', 'UTF-8', $rDealNo) }}">			<!-- (신용카드공통)거래번호 -->

<input type=hidden name=rCardNm value="{{ iconv('EUC-KR', 'UTF-8', $rCardNm) }}">	<!-- (안심클릭,일반사용)카드사명 -->
<input type=hidden name=rMembNo value="{{ iconv('EUC-KR', 'UTF-8', $rMembNo) }}>">	<!-- (안심클릭,일반사용)가맹점번호 -->
<input type=hidden name=rAquiCd value="{{ iconv('EUC-KR', 'UTF-8', $rAquiCd) }}">		<!-- (안심클릭,일반사용)매입사코드 -->
<input type=hidden name=rAquiNm value="{{ iconv('EUC-KR', 'UTF-8', $rAquiNm) }}">	<!-- (안심클릭,일반사용)매입사명 -->

<!-- 계좌이체 결제 사용 변수 -->
<input type=hidden name=ICHE_OUTBANKNAME value="{{ iconv('EUC-KR', 'UTF-8', $ICHE_OUTBANKNAME) }}">		<!-- 이체은행명 -->
<input type=hidden name=ICHE_OUTBANKMASTER value="{{ iconv('EUC-KR', 'UTF-8', $ICHE_OUTBANKMASTER) }}">	<!-- 이체계좌예금주 -->
<input type=hidden name=ICHE_AMOUNT value="{{ iconv('EUC-KR', 'UTF-8', $ICHE_AMOUNT) }}">					<!-- 이체금액 -->

<!-- 핸드폰 결제 사용 변수 -->
<input type=hidden name=rHP_HANDPHONE value="{{ iconv('EUC-KR', 'UTF-8', $rHP_HANDPHONE) }}">		<!-- 핸드폰번호 -->
<input type=hidden name=rHP_COMPANY value="{{ iconv('EUC-KR', 'UTF-8', $rHP_COMPANY) }}">			<!-- 통신사명(SKT,KTF,LGT) -->
<input type=hidden name=rHP_TID value="{{ iconv('EUC-KR', 'UTF-8', $rHP_TID) }}">					<!-- 결제TID -->
<input type=hidden name=rHP_DATE value="{{ iconv('EUC-KR', 'UTF-8', $rHP_DATE) }}">				<!-- 결제일자 -->

<!-- ARS 결제 사용 변수 -->
<input type=hidden name=rARS_PHONE value="{{ iconv('EUC-KR', 'UTF-8', $rARS_PHONE) }}">			<!-- ARS번호 -->

<!-- 가상계좌 결제 사용 변수 -->
<input type=hidden name=rVirNo value="{{ iconv('EUC-KR', 'UTF-8', $rVirNo) }}">					<!-- 가상계좌번호 -->
<input type=hidden name=VIRTUAL_CENTERCD value="{{ iconv('EUC-KR', 'UTF-8', $VIRTUAL_CENTERCD) }}">	<!--입금가상계좌은행코드(우리은행:20) -->

<!-- 이지스에스크로 결제 사용 변수 -->
<input type=hidden name=ES_SENDNO value="{{ iconv('EUC-KR', 'UTF-8', $ES_SENDNO) }}">				<!-- 이지스에스크로(전문번호) -->
